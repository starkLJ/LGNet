    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            if metric == 'proposal':
                ...
                tmp_iouThs = cocoEval.params.iouThrs 
                cocoEval.params.iouThrs = [0.7]
                
                cocoEval.evaluate()
                cocoEval.accumulate()
                cocoEval.summarize()
                print("Average Precision at IoU 0.7 (AP70): {:.3f}".format(cocoEval.stats[0]))
                print("Average Recall at IoU 0.7 (AR70): {:.3f}".format(cocoEval.stats[1]))
                cocoEval.params.iouThrs = tmp_iouThs
                cocoEval.evaluate()
                cocoEval.accumulate()
                cocoEval.summarize()
...
@DATASETS.register_module()
class UAVDTSingle(UAVDTDataset):
    CLASSES = ('vehicle')
    def load_annotations(self, ann_file):
        self.coco = COCO(ann_file)
        self.cat_ids = self.coco.getCatIds(catNms=self.CLASSES)
